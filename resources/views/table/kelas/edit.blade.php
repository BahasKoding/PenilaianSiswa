                ...
                <div class="card-body">
                    <form action="/table/kelas/store" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-floating mb-3">
                            ...
                            <input required name="nama_kelas" type="text" value="{{ $kelas->nama_kelas }}" class="form-control" id="floatingInput3">
                        </div>
                        <div class="form-floating mb-3">
                            ...
                            <select class="form-select form-control select2" aria-label="Default select example" name="jurusan_id" required>
                                @foreach ($jurusan as $datas)
                                    @if ($kelas->jurusan_id == $datas->id)
                                        <option value="{{ $datas->id }}" selected>{{ $datas->nama_jurusan }}</option>
                                    @else
